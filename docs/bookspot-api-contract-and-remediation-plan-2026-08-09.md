...
| HD-05 (E3) | Cancellation cut-off/fees and no-show consequences. | Either party may cancel before start, no fee; `no_show` recorded but inert. | Fees/consequences deferred; state safety not blocked. |
...
| 409 | `email_already_registered`, `booking_slot_conflict`, `booking_state_conflict`, `booking_configuration_conflict`, `idempotency_key_reused`, `idempotency_window_expired` | Atomic uniqueness/domain/concurrency conflict or an intentionally expired replay representation. |
...
2. Generate 32 random bytes with a CSPRNG; base64url without padding. Store only SHA-256/HMAC digest, account, generation, issue/expiry, and consumed state.
...
6. Reset POST atomically conditions on latest generation, unexpired and unused; writes a hash from the common BCrypt service, consumes token, increments security/session version, and invalidates all reset capabilities.
...
9. Send confirmation and record a redacted audit event after success.
...
The worker consumes the same queue shape for every request, decrypts and re-normalizes the email, derives the exact §9.2 `ClaimKey`, performs one strongly consistent base-table claim read and referenced Profile read, and acknowledges only after bounded processing. No recovery GSI or alternate identity index exists. For a known account it runs the three-action issue transaction below, copying the queued source hashes into the issue audit item, and then enqueues mail asynchronously; for an unknown/broken claim it performs the same HMAC/token-generation/serialization work in memory, writes no capability/audit/mail, and acknowledges. Worker retries/DLQ behavior is private and cannot alter the already released public tuple. Mail enqueue failure is retried from a non-secret internal issue event and never changes the public response. Access to queue/DLQ is restricted, encryption at rest/in transit is mandatory, and expired requests are discarded without identity-dependent public effects.
...
ProfileId, Generation (number), IssuedAtUtc, ExpiresAtUtc, ConsumedAtUtc|null
...
EventType = RESET_CAPABILITY_ISSUED | PASSWORD_RESET_COMPLETED
...
**Issue transaction:** in the asynchronous recovery worker, a known account issues generation `g=LatestGeneration+1` in one three-action `TransactWriteItems`: conditionally update/create account-state from expected generation to `g`; put the digest capability with `attribute_not_exists(CapabilityKey)` and 30-minute expiry; put exactly one `RESET_CAPABILITY_ISSUED` auth-audit item conditioned absent. A concurrent issuer that loses commits zero capability/account-state/audit actions, rereads/retries with a new token/generation/event ID, and only its eventual committed winner has one audit event; an abandoned/exhausted loser has zero durable auth-audit events and increments only non-PII security contention telemetry. Only the greatest committed generation validates. Older capability rows need not be rewritten because generation comparison supersedes them. Unknown/broken-claim requests take the worker path defined above and create no durable capability, auth audit, or mail.

**Validate:** digest the syntactically untrusted token, strongly read capability and account state, and return success only if capability exists, generation equals latest, `ConsumedAtUtc` absent, and `now < ExpiresAtUtc`. It never mutates or extends expiry.

**Consume transaction:** precompute BCrypt outside the transaction; then one four-action `TransactWriteItems` conditionally (a) updates the Profile password hash and increments `SecurityVersion` exactly once from the version read, (b) updates capability `ConsumedAtUtc` only when absent/unexpired/generation matches, (c) advances account-state generation to `g+1` conditioned on `LatestGeneration=g`, invalidating every capability, and (d) puts exactly one `PASSWORD_RESET_COMPLETED` auth-audit item conditioned absent. If any condition loses, no password/session/audit/capability state changes: the winning consume has exactly one durable audit event and every race loser has zero, with only aggregate non-PII invalid-token/contention telemetry. Cache invalidation occurs synchronously before returning success. Transaction outcome, not a preflight read, decides success.

For **both** validate and consume, unknown, empty, malformed, superseded, expired, used, digest-pepper mismatch, and race-losing tokens return this byte-equivalent tuple (apart from `traceId` and `instance`, which MUST be omitted here to preserve byte equivalence): HTTP `400`; `Content-Type: application/problem+json`; `Cache-Control: no-store, max-age=0`; `Pragma: no-cache`; no `Retry-After`, `WWW-Authenticate`, `Set-Cookie`, `ETag`, or lifecycle-dependent header; body exactly `{"type":"https://bookspot.example/problems/reset-token-invalid","title":"Reset token invalid","status":400,"detail":"The reset capability is invalid.","code":"reset_token_invalid"}` encoded UTF-8 without insignificant-property variation. Response length and timing are padded to the same configured bucket. Only a valid validate returns `200 {"valid":true}`; only a winning consume returns the exact 200 `ResetPasswordSuccessResponse`.

**Legacy migration:** provision `auth_capabilities`, `auth_audit`, `auth_abuse_counters`, encrypted `auth_recovery_requests`, its DLQ, and the recovery worker/mail-issue retry channel in deployment and local integration infrastructure before enabling new auth writers; verify exact keys/message schema, 24-hour/7-day queue retention, TTL policy (capability/counter cleanup only; no auth-audit TTL), IAM, encryption at rest/in transit, point-in-time recovery where applicable, and alarms. Deploy dual-read/no-legacy-write code, create/backfill Profile security versions and account-state generations, then invalidate rather than copy every plaintext-keyed legacy reset row. During a maximum 30-minute drain, legacy tokens are uniformly treated as invalid and users request a new token; no plaintext token is moved into the new table. Disable/delete the old writer first, verify zero new old-table writes, wait maximum legacy expiry + safety margin, revoke table access, export only non-secret migration counts, then delete the old table under a separately approved retention task. Reconciliation asserts every committed reset generation has exactly one issue audit, every successful security-version reset increment has exactly one completion audit, no losing/invalid/unknown attempt has a durable event, and no audit exists without its corresponding committed state. Tests prove byte-equivalent success/failure tuples, identical known/unknown queue shape, no synchronous identity lookup, digest-only capability storage, latest-generation supersession, issue/consume races, transaction rollback at every action, exact action counts, one password/version/audit mutation, audit redaction/retention/access, and no legacy plaintext capability in logs/backups/new storage.
...
| `pending` | cancel | `cancelled` | client or owning provider | Terminal; release all cells. |
| `confirmed` | cancel | `cancelled` | client or owning provider | Before start under interim HD-05; release all cells. |
...
- **Cancel:** Put request absent; update pending/confirmed booking to cancelled/version+1 under timing policy; conditionally delete every old slot owned by booking; Put audit absent.
...
- Issue/consume fault injection at every transaction action yields no partial capability/profile/security-version/auth-audit state. Exact three/four-action counts, winner-one/loser-zero audit semantics, reconciliation, seven-year retention/no TTL, IAM/redaction, provisioning parity, and no orphan audit/state are asserted. Migration proves no plaintext legacy row is copied, old writer is disabled, and new capability table contains digest keys only.
...
- Pending and confirmed block; terminal release is atomic.
