...
| Booking `Idempotency-Key` | Required for every booking mutation; one and only one header value, 16–128 printable ASCII bytes, no surrounding whitespace/control characters. Missing, repeated/comma-folded, or malformed -> `400 validation_failed`, key `idempotencyKey`; never trim, coerce, log, or echo. |
| Reset `Idempotency-Key` | Required only on `POST /auth/reset-password`; one and only one header value containing exactly 32 unpadded base64url characters (`A-Z a-z 0-9 - _`) that decode to exactly 24 bytes generated by the client CSPRNG. Because 32 base64url characters encode exactly 24 bytes with no pad bits, valid-alphabet input in this fixed length has one canonical representation. This structurally supplies 192 random bits; the server validates alphabet/length/decoding, not statistical entropy. Missing, empty, padded, repeated/comma-folded, shorter, longer, or non-alphabet values receive the exact generic §9.1 reset-invalid tuple with no field detail. Never trim, coerce, log, echo, or reuse across reset attempts. |
...
// POST /auth/reset-password additionally requires the exact reset Idempotency-Key header in §5.3.
...
| `POST /auth/reset-password` | Public | reset `Idempotency-Key` header + `ResetPasswordRequest -> ResetPasswordSuccessResponse` | Header is mandatory and follows the exact reset-specific §5.3 contract. 200 winner body `{"message":"Password reset completed.","success":true}`; exact 400 tuple in §9.1 for every unusable/race-losing token or missing/malformed/different key; 429/503 per §5.4. |
...
AeadAlgorithm = AES-256-GCM; AeadEnvelopeVersion = 1
NonceBase64Url; CiphertextBase64Url; AuthenticationTagBase64Url
EncryptedDataKeyBase64Url; KmsKeyId; EncryptionContextVersion = 1
```

Before each transaction submission, the auth worker calls KMS `GenerateDataKey` on the dedicated auth-mail key with exactly these case-sensitive string encryption-context pairs: `BookSpotPurpose=AuthMail`, `DeliveryKey=<exact DeliveryKey>`, `EventType=<exact EventType>`, `ProfileId=<exact ProfileId>`, `CapabilityGeneration=<base-10 generation without leading zeroes>`, and `SchemaVersion=1`. It receives one fresh 256-bit plaintext data key plus its KMS ciphertext blob. The same complete context is mandatory on KMS `Decrypt`; missing, additional, differently cased, or unequal context fails closed. `KmsKeyId` identifies the configured key ARN/version and is verified against the allow-list before decrypt.

The worker generates a fresh uniformly random **96-bit nonce for every payload** and performs AES-256-GCM with a 128-bit authentication tag. The persisted v1 envelope has no implicit provider framing: `NonceBase64Url` is the 12-byte nonce, `CiphertextBase64Url` is ciphertext bytes **excluding** the tag, `AuthenticationTagBase64Url` is the separate 16-byte tag, and `EncryptedDataKeyBase64Url` is the opaque KMS ciphertext blob; each uses canonical unpadded base64url. The AEAD AAD bytes are UTF-8 of JCS/RFC 8785 canonical JSON containing exactly `{"capabilityGeneration":<number>,"deliveryKey":"<JSON-escaped exact value>","eventType":"<JSON-escaped exact value>","profileId":"<JSON-escaped exact value>","schemaVersion":1}` with keys in the shown canonical order and no additional properties. `AeadAlgorithm`, `AeadEnvelopeVersion`, item identity fields, and AAD values must match before decrypt; v1 readers reject unknown versions rather than guessing framing.

For `RESET_CAPABILITY_DELIVERY`, the encrypted plaintext is JCS canonical JSON containing exactly the raw token, normalized registered destination, reset-link template/version and public expiry; for `PASSWORD_RESET_CONFIRMATION`, it contains the destination and confirmation template/version but no token, password, hash, IP, or session value. Plaintext data keys, nonce-generation state, and payloads exist only in bounded worker memory and are zeroed/released after use; none appears in logs, traces, metrics, exceptions, backups, DLQs, provider metadata, or audit. Only the issue/consume transaction role may generate/encrypt/put, and only the isolated delivery-worker role may read/decrypt/update; support, API request handlers, clients, and general application roles have no decrypt or table-read permission. DynamoDB encryption and point-in-time recovery remain enabled, but they do not replace payload envelope encryption.
...
- Table-driven validation exercises every §5.3 field/query/header at omitted/null/blank/min/max/one-beyond boundaries, decimal/cardinality/duplicate/unknown-property rules, empty PATCH, repeated/unknown query keys, and cursor tampering; exact stable field keys/tokens are asserted. Booking and reset idempotency headers use their distinct named rows: reset tests cover missing/empty, 31/32/33 characters, padded and non-alphabet input, duplicate/comma-folded values, exact 24-byte decoding, different-key retry, same-key commit-loss/process-restart replay, and proof that malformed headers receive only the generic reset-invalid tuple.
