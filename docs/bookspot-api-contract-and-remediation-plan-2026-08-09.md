...
| `pending` | cancel | `cancelled` | client or owning provider | Require server `now < startTime`; terminal; release all cells. |
| `confirmed` | cancel | `cancelled` | client or owning provider | Require server `now < startTime`; terminal; release all cells. |
...
- **Cancel:** capture one server UTC `actionNow`; Put request absent; update pending/confirmed booking to cancelled/version+1 only when `actionNow < startTime`; conditionally delete every old slot owned by booking; Put audit absent. Equality or later fails 409 `booking_cancellation_window_closed` for an authorized party.
...
- Issue/consume fault injection at every transaction action yields no partial capability/profile/security-version/auth-audit/delivery state. Exact four/five-action counts, winner-one/loser-zero audit and delivery semantics, reconciliation, seven-year audit retention/no TTL, outbox payload encryption/erasure/TTL, IAM/KMS-context isolation, provisioning parity, and no orphan audit/state/delivery are asserted. Crash injection covers before transaction, every transaction action, commit-response loss, claim, decrypt, provider acceptance, and delivery acknowledgement; it proves every committed issue is eventually provider-accepted before deadline or atomically revoked, no uncommitted token is mailed, ack loss can only duplicate the identical capability, superseded mail never validates, terminal payload/key fields are erased, confirmation handoff survives consume commit, and reconciler/lease/DLQ paths converge. Migration proves no plaintext legacy row is copied, old writer is disabled, and the capability table contains digest keys only.
...
- Pending and confirmed block; terminal release is atomic. Pending and confirmed cancellation both require server `now < startTime`; before/equal/after boundary tests for each actor/source assert equality and later return 409 `booking_cancellation_window_closed` with no mutation.
